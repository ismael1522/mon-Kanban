...
export default function TaskForm({ boardId, onCreated }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ background: 'white', padding: '1.5rem', borderRadius: '10px', marginBottom: '1.5rem', border: '1px solid #E2E8F0' }}>
      ...
      <input placeholder='Titre de la tâche *' value={title} onChange={e => setTitle(e.target.value)} required style={{ ...inputStyle, marginBottom: '0.75rem' }} />
      <textarea placeholder='Description (optionnelle)' value={description} onChange={e => setDescription(e.target.value)} rows={3} style={{ ...inputStyle, marginBottom: '0.75rem', resize: 'vertical' }} />
